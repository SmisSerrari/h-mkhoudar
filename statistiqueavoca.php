...
if(isset($_POST['filter'])){
    ...
    $query1 =mysqli_query($conn,"SELECT tabavoca,COUNT(*),tabreqGen, SUM(tabnote) FROM tabligh WHERE tabavoca = '$tabavoca' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' GROUP BY tabreqGen ");
    
    $count = mysqli_num_rows($query1);

    $query2 =mysqli_query($conn,"SELECT tabavoca,COUNT(*),tabdatetabligh,SUM(tabnote) FROM tabligh WHERE tabavoca = '$tabavoca' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' GROUP BY tabdatetabligh ");
    $count1 = mysqli_num_rows($query2);
    
    $query3 =mysqli_query($conn,"SELECT tabavoca,tabgenre,COUNT(*) FROM tabligh WHERE tabavoca = '$tabavoca' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' GROUP BY tabgenre ");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3>احصائيات المحامون  حسب الملاحظات  و تاريخ التبليغ </h3>
                        </div>
                        <!-- /.panel-heading -->
                        ...
                        <input type="date" name="from_date" class="form-control" placeholder="من">
                            ...
                            <div class="col-lg-3">
                            <label>المحامي</label>
                               <select class="form-control" name="tabavoca">
                                <option value=""> </option>
                                <?php 
                                    
                                    $query = "SELECT * FROM avocas";
...
{       
        echo '<CENTER>';
        echo'<h3>   المحامي  (ة) : الاستاذ (ة)  '.$tabavoca.' <h3>';
        echo '<h4>    من  :   '.$from_date.'     إلى  :   '.$to_date.' <h4>';
        
        echo '</CENTER>';
        echo'<h4><B> -إحضاء الملاحظات </B><h4>';
    ...
    while($val5=mysqli_fetch_array($query1))
    {
        ...
        echo '<tr>';                
                           //
                           echo '<td>'; echo $val5['tabavoca'];echo '</td>';
...
{       
    echo'<h4><B> -إحصاء الإجراءات </B><h4>';
    while($val7=mysqli_fetch_array($query3))
    {

        echo '<tr>';                
                           //
                           echo '<td>'; echo $val7['tabavoca'];echo '</td>';
                           echo '<td>'; echo $val7['tabgenre'];echo '</td>';
                           echo '<td>'; echo $val7['COUNT(*)'];echo '</td>';
...
{       
    ...
    while($val6=mysqli_fetch_array($query2))
    {
        ...
        echo '<tr>';                
                           //
                           echo '<td>'; echo $val6['tabavoca'];echo '</td>';
