            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="col-lg-3">  <a href="changeclerctab.php" class="btn btn-info btn-lg" style="margin:15px; width:50%"  >تغيير الكاتب </a>
                        ...
                        $NbrCol = 100000;
                        // requête
                        $query = 'SELECT * FROM tabligh WHERE encour!=1 AND donne=1';
                        $result = mysqli_query($conn,$query);;
                        // -------------------------------------------------------      
                        ...
                        { 
                           ...
                           if(($val['tabgenre']=="جلسة") || ($val['tabgenre']=="إعذار") || ($val['tabgenre']=="بيوعات عقارية" )){
                            
                           echo '<td>';

                           
                           $datev = $val['num_donne'];
                           $dateretard = date('Y-m-d', strtotime($datev.'+'.$val['heureret']));
                           
                            $query5 = "INSERT INTO tabligh (dateretard) VALUES ($dateretard)";
                           mysqli_query($conn,$query5);
                           echo $dateretard;
                           echo '</td>';
                            
                           }else if($val['tabgenre']=="انذار مباشر SPH" ){
                            echo '<td>';
                           
                           $dateM2 = date('Y-m-d', strtotime($datev.'+'.$val['heureret']));
                           ...
                           echo '<td>';
                           $datev = $val['num_donne'];
                           $dateM3 = date('Y-m-d', strtotime($datev.'+48 hour'));
                           echo $dateM3;
                           echo '</td>';
                            }
 ...
 if(!empty($_POST['encour'])) {
    $counter = 0;
        ...
        $counter = $counter+1;
            ...
            $query = "UPDATE tabligh SET encour=1 WHERE idtab = $value";
            mysqli_query($conn,$query);
            $query2 = "UPDATE tabligh SET num_encour=NOW() WHERE idtab = $value";
            mysqli_query($conn,$query2);
              
        }
        $num_donne =  $num_donne + 1;
          header('Location: tabcoure.php?counter='.$counter.'');
...
if(!empty($_POST['donne'])) {
    $counter = 0;
        ...
        $counter = $counter+1;
            ...
            $query3 = "UPDATE tabligh SET donne=0 WHERE idtab = $value";
            mysqli_query($conn,$query3);
            $query4 = "UPDATE tabligh SET encour=0 , num_encour = 0 WHERE idtab = $value";
            mysqli_query($conn,$query4 );
              
        }
        $num_donne =  $num_donne + 1;
          header('Location: bureautab.php?counter='.$counter.'');
