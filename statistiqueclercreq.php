...
if(isset($_POST['filter'])){
    ...
    $query1 =mysqli_query($conn,"SELECT tabclerc,COUNT(*),tabreqGen, SUM(tabnote) FROM tabligh WHERE tabclerc = '$tabclerc' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' GROUP BY tabreqGen ");
    
    $count = mysqli_num_rows($query1);

    $query2 =mysqli_query($conn,"SELECT tabclerc,COUNT(*),tabdatetabligh,SUM(tabnote) FROM tabligh WHERE tabclerc = '$tabclerc' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' GROUP BY tabdatetabligh ");
    $count1 = mysqli_num_rows($query2);
    
    $query3 =mysqli_query($conn,"SELECT SUM(tabnote),COUNT(*) FROM tabligh WHERE tabclerc = '$tabclerc' AND tabdatetabligh BETWEEN '$from_date' AND '$to_date' ");
...
<body  style="font-family:tahoma;">
    ...
    <div id="wrapper">
       ...
       <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"> إحصائيات المحامون  </h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3>احصائيات الكتاب  حسب الملاحظات  و تاريخ التبليغ </h3>
                        </div>
                        <!-- /.panel-heading -->
                        ...
                        <div class="col-lg-3">
                             ...
                             <input type="submit" name="filter" value="تصفية" class="btn btn-success btn-lg" style="margin:15px; width:50%"  >
...
{       
        ...
        echo '</CENTER>';
        echo'<h4><B> -الإحضاء الملاحظات </B><h4>';
    ...
    while($val5=mysqli_fetch_array($query1))
    {
        ...
        echo '<tr>';                
                           ...
                           echo '<td>'; echo $val5['SUM(tabnote)'];echo '</td>';
                           //echo '<td>';
                                ...
                                <table class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><CENTER>مجموع  الإجراءات</CENTER></th>
                                            <th><CENTER>مجموع النقط <CENTER></th>                                       
                                        </tr>
                                    </thead>
...
{       
    while($val=mysqli_fetch_array($query3))
    {

        echo '<tr>';                
                           //
                           
                           echo '<td><CENTER><h3>';echo $val['COUNT(*)'];echo '</h3></td>';
                           echo '<td><CENTER><h3>'; echo $val['SUM(tabnote)'];echo '</h3></td>';
                           //echo '<td>';
                           //echo '<input type="checkbox" name="donne[]" value="'.$val['idtab'].'"/>';echo '</td>';
...
{       
    echo'<h4><B> - إحصاء حسب  تاريخ التبليغ </B><h4>';
    while($val6=mysqli_fetch_array($query2))
    {

        echo '<tr>';                
                           //
                           echo '<td>'; echo $val6['tabclerc'];echo '</td>';
                           echo '<td>'; echo $val6['tabdatetabligh'];echo '</td>';
                           echo '<td>'; echo $val6['COUNT(*)'];echo '</td>';
                           echo '<td>'; echo $val6['SUM(tabnote)'];echo '</td>';
                           //echo '<td>';
                           //echo '<input type="checkbox" name="donne[]" value="'.$val['idtab'].'"/>';echo '</td>';
